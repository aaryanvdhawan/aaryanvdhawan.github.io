...
While not mentioned in the paper, we assume there is a register before the binary adder to avoid metastability issues.
...
```
Rather than implementing hardware to compute the potention/depression values with the 2 equations above, the authors use two Look-Up Tables (LUTs), one for potentiation and one for depression, to easily compute $$\Delta w$$ as there is only a certain range of possible values. $$\Delta w$$ is then added to synaptic connection, read from a memory block, using a simple adder. The synaptic connection is then written back to the memory block, completing the learning process for that given connection. The LUT and Adder are implemented as follows:
```verilog
...
![Optimized STDP Learning Engine](/images/Optimized_STDP_LE.png)
...
```
The potentiation LUT is now reduced to 16 entries, as the depression values are not needed. The adder now takes in the potentiation value and a signed flag, which determines if the value should be added or subtracted from the current weight. The adder can be implemented as follows:
```verilog
always_comb begin
